---
title: "git使用"
date: 2024-09-24
categories: [git]
tags: [git]
excerpt: "git命令"
---

> 参考
>
> - [廖雪峰git教程](https://liaoxuefeng.com/books/git/introduction/index.html)
> - [Git 如何移除一个子模块](https://geek-docs.com/git/git-questions/26_git_how_do_i_remove_a_submodule.html)

## 版本管理

```mermaid
flowchart LR
    %% 样式定义
    classDef localBox fill:#f9fafb,stroke:#9ca3af,stroke-width:2px,stroke-dasharray: 5 5;
    classDef remoteBox fill:#eff6ff,stroke:#3b82f6,stroke-width:2px,stroke-dasharray: 5 5;
    
    classDef ws fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a;
    classDef sa fill:#fef3c7,stroke:#d97706,stroke-width:2px,color:#78350f;
    classDef lr fill:#d1fae5,stroke:#059669,stroke-width:2px,color:#064e3b;
    classDef rr fill:#ede9fe,stroke:#7c3aed,stroke-width:2px,color:#4c1d95;

    subgraph Local_Machine ["💻 本地环境 (Local Machine)"]
        direction LR
        WS["📂 <b>Work Space</b><br/>(工作区)<br/><i>Modified / Untracked</i>"]
        SA["📦 <b>Stage Area</b><br/>(暂存区)<br/><i>Staged</i>"]
        LR["🗄️ <b>Local Repo</b><br/>(本地版本库)<br/><i>Committed (本地分支)</i>"]
        
        WS -- "1. git add" --> SA
        SA -- "2. git commit" --> LR
        LR -- "git restore<br/>(丢弃工作区修改)" --> WS
        SA -- "git restore --staged<br/>(取消暂存)" --> WS
    end

    subgraph Remote_Server ["☁️ 远程服务器 (Remote Server, e.g., GitHub/GitLab)"]
        direction TB
        RR["🌐 <b>Remote Repo</b><br/>(远程仓库)<br/><i>远程分支 (origin/main)</i>"]
    end

    %% 跨环境交互
    LR -- "3. git push<br/>(推送本地提交到远程)" --> RR
    RR -- "4. git fetch<br/>(拉取远程更新到本地跟踪分支)" --> LR
    RR -- "5. git pull<br/>(拉取并自动合并到工作区)" --> WS
    
    RR -. "0. git clone<br/>(首次克隆整个项目)" .-> WS

    %% 应用样式
    class Local_Machine localBox;
    class Remote_Server remoteBox;
    class WS ws;
    class SA sa;
    class LR lr;
    class RR rr;
```

### 概念

#### work space(工作区)

`work space`(工作区)是本地文件系统中存储库目录, 用于存放源代码

开发者在`work space`内进行代码编辑、修改和删除等操作, 文件所有更改都会反映在`work space`

`work space`中, 文件有三种状态:

- `modified`(已修改), 文件已修改, 还未添加到`stage area`

- `staged`(已暂存), 文件已修改, 并且已添加到`stage area`, 准备提交到版本库

- `committed`(已提交), 文件已提交到版本库

```mermaid
flowchart LR
    %% 区域背景样式
    classDef bg_ws fill:#eff6ff,stroke:#3b82f6,stroke-width:2px,stroke-dasharray: 5 5;
    classDef bg_sa fill:#fffbeb,stroke:#f59e0b,stroke-width:2px,stroke-dasharray: 5 5;
    classDef bg_lr fill:#ecfdf5,stroke:#10b981,stroke-width:2px,stroke-dasharray: 5 5;

    %% 状态节点样式
    classDef state_u fill:#f8fafc,stroke:#94a3b8,stroke-width:2px,color:#475569;
    classDef state_m fill:#fee2e2,stroke:#ef4444,stroke-width:3px,color:#991b1b,font-weight:bold;
    classDef state_s fill:#fef3c7,stroke:#f59e0b,stroke-width:3px,color:#92400e,font-weight:bold;
    classDef state_c fill:#d1fae5,stroke:#10b981,stroke-width:3px,color:#065f46,font-weight:bold;

    subgraph WS ["📂 Work Space (工作区 - 本地文件系统)"]
        direction TB
        U["📄 <b>Untracked</b> (未跟踪)<br/><i>新创建的文件，Git 尚未纳管</i>"]
        M["📝 <b>Modified</b> (已修改)<br/><i>文件内容已更改，但还未添加到暂存区</i>"]
    end

    subgraph SA ["📦 Stage Area (暂存区 / Index)"]
        direction TB
        S("📋 <b>Staged</b> (已暂存)<br/><i>文件已修改并添加到暂存区，准备提交</i>")
    end

    subgraph LR ["🗄️ Local Repository (本地版本库)"]
        direction TB
        C{{"✅ <b>Committed</b> (已提交)<br/><i>文件快照已安全保存至本地数据库</i>"}}
    end

    %% 正向流转 (提交工作流)
    U -- "1. 首次 <code>git add</code>" --> S
    M -- "2. <code>git add</code><br/>(将修改放入暂存区)" --> S
    S -- "3. <code>git commit</code><br/>(将暂存区永久保存)" --> C

    %% 逆向与修改流转 (撤回与再编辑)
    C -- "4. 在工作区再次编辑该文件" --> M
    S -- "5. <code>git restore --staged</code><br/>(取消暂存，退回工作区)" --> M
    M -- "6. <code>git restore</code><br/>(丢弃工作区修改，恢复原状)" --> C

    %% 应用样式
    class WS bg_ws;
    class SA bg_sa;
    class LR bg_lr;
    
    class U state_u;
    class M state_m;
    class S state_s;
    class C state_c;
```

#### stage area (暂存区)

`stage area`(暂存区)本质是数据库

`stage area`充当缓冲, 将`work space`的修改先保存到`stage area`, 再提交到`local repository`

#### local repository (本地仓库)

`local repository`位于开发者的计算机中, 包含所有的项目提交历史修改后文件提交到`local repository`, 成为版本历史一部分

#### remote repository (远程仓库)

远程仓库存储在远程服务器上, 用于备份、共享和协作工作, 允许开发者推送本地更改或拉取他人更改

拥有三种操作:

- `clone`: 克隆远程仓库到本地

- `push`: 将本地更改推送到远程仓库

- `pull`: 将远程更改拉取到本地

### 初始化

将当前目录初始化为`local repository`

```sh
git init
```

执行后自动创建隐藏目录`.git/`, 用于管理版本库

`.git/`位于`local repository`根目录下, 包含仓库所有元数据和信息, 用于跟踪和管理版本控制

初始化后, 当前目录及子目录和文件都处于`work space`(工作区)

- 示例, 将`git_test/`初始化为`local repository`

```sh
git init
```

终端显示

```sh
Initialized empty Git repository in /home/dmjcb/Documents/code/git_test/.git/
```

#### 状态

查看`work space`中文件状态

```sh
git status
```

- 未添加文件, 显示

```sh
On branch master

No commits yet

nothing to commit (create/copy files and use "git add" to tarck)
```

#### 修改

新增文件, 状态为`untracked files`, 终端里显示为红色, 表示还未添加到`stage area`

#### 撤销

- 撤销`work space`内文件修改

将文件恢复到上一次提交状态

```sh
git restore 文件名
```

- 撤销`work space`内所删除文件

从上一次提交中恢复被删除文件到`work space`

```sh
git checkout -- 文件名
```

### 添加

将文件从`work space`转移到`stage area`

```sh
git add 文件名(如果为 . 表示全部文件)
```

#### 撤回

将已添加至`stage area`的文件撤回至`work space`

```sh
git rm --cached 文件名
```

### 提交

将`stage area`文件提交至`local repository`

```sh
git commit (-m "commit信息")
```

- 示例, 提交main.cpp

若未设置邮箱用户名, 需先设置

```sh
git config --global user.email "邮箱"
git config --global user.name "用户名"
```

设置完成后正常提交

#### 查看记录

查看每次提交记录, 包含`commit ID`、作者、提交时间信息

```sh
git log
```

示例显示

```sh
commit 69c26f0f7489e0fed9d22a83d12b43d6ddfbd95e (HEAD -> main, origin/main, origin/HEAD)
Author: dmjcb <dmjcb@outlook.com>
Date:   Wed Jun 10 21:09:58 2026 +0800

    modify

commit b02b3331ad2a4820f4b7d3ea47c3d78f8a5ed58a
Author: dmjcb <dmjcb@outlook.com>
Date:   Mon Jun 8 23:07:55 2026 +0800

    update
:
```

#### 修改commit内容

假设需在`commit`后修改内容提交内容

- 修改最近一次`commit`

```sh
git commit --amend
```

### 回退

```mermaid
flowchart TB
    %% 区域背景样式 (与上图一致)
    classDef bg_ws fill:#eff6ff,stroke:#3b82f6,stroke-width:2px,stroke-dasharray: 5 5;
    classDef bg_sa fill:#fffbeb,stroke:#f59e0b,stroke-width:2px,stroke-dasharray: 5 5;
    classDef bg_lr fill:#ecfdf5,stroke:#10b981,stroke-width:2px,stroke-dasharray: 5 5;

    %% 状态节点样式 (与上图一致)
    classDef state_m fill:#fee2e2,stroke:#ef4444,stroke-width:3px,color:#991b1b,font-weight:bold;
    classDef state_s fill:#fef3c7,stroke:#f59e0b,stroke-width:3px,color:#92400e,font-weight:bold;
    classDef state_c fill:#d1fae5,stroke:#10b981,stroke-width:3px,color:#065f46,font-weight:bold;
    classDef state_c_prev fill:#a7f3d0,stroke:#059669,stroke-width:3px,color:#064e3b,font-weight:bold,stroke-dasharray: 5 5;

    subgraph WS ["📂 Work Space (工作区)"]
        direction TB
        M["📝 <b>Modified</b> (已修改)<br/><i>文件已更改，未暂存</i>"]
    end

    subgraph SA ["📦 Stage Area (暂存区)"]
        direction TB
        S("📋 <b>Staged</b> (已暂存)<br/><i>已添加到暂存区，准备提交</i>")
    end

    subgraph LR ["🗄️ Local Repository (本地版本库)"]
        direction TB
        C{{"✅ <b>Committed</b> (当前已提交)<br/><i>当前版本 (HEAD) 已安全保存</i>"}}
        C_Prev{{"⏪ <b>Committed</b> (上一版本)<br/><i>HEAD^ 或 指定 commit_id</i>"}}
    end

    %% ================= 回退操作 (逆向流转，使用虚线) =================

    %% 1. 未暂存回退
    M -. "1️⃣ <b>未暂存回退</b><br/><code>git restore &lt;file&gt;</code><br/><i>(或 git checkout --)</i><br/>丢弃工作区修改，恢复至干净状态" .-> C

    %% 2. 已暂存回退
    S -. "2️⃣ <b>已暂存回退</b><br/><code>git restore --staged &lt;file&gt;</code><br/><i>(或 git reset HEAD)</i><br/>仅撤回 add 操作，保留文件修改内容" .-> M

    %% 3. 已 commit 回退 (--soft)
    C -. "3️⃣ <b>已 commit 回退 (--soft)</b><br/><code>git reset --soft HEAD^</code><br/>撤销 commit，<b>不撤销</b> add<br/>文件退回暂存区，保留修改" .-> S
    
    %% 4. 已 commit 回退 (--mixed)
    C -. "4️⃣ <b>已 commit 回退 (--mixed)</b><br/><code>git reset --mixed HEAD^</code><br/>撤销 commit，<b>并撤销</b> add<br/>文件退回工作区，保留修改" .-> M
    
    %% 5. 已 commit 回退 (--hard)
    C -. "5️⃣ <b>已 commit 回退 (--hard)</b><br/><code>git reset --hard HEAD^</code><br/>撤销 commit 和 add，<b>强制删除</b>工作区改动<br/>彻底恢复至上一版本" .-> C_Prev

    %% 应用样式
    class WS bg_ws;
    class SA bg_sa;
    class LR bg_lr;
    
    class M state_m;
    class S state_s;
    class C state_c;
    class C_Prev state_c_prev;
```
"

设原始test.txt

```c++
origin origin origin origin origin
```

设修改后test.txt

```c++
modify modify modify modify modify
```

#### 未暂存回退

用于已修改但未添加到`stage area`(即未执行`git add`时)回退

文件会恢复至修改前状态

```sh
git checkout -- [文件名](为 . 时表示全部文件)
```

#### 已暂存回退

用于已修改并添加到`stage area`(已执行`git add`)回退

注意! `git reset`只撤回添加操作, 不会撤回文件修改

- 回退指定文件

```sh 
git reset HEAD 文件名
```

- 回退所有修改

```sh
git reset
```

执行后文件被撤回到`work space`, 但修改后内容不变

#### 已commit退回

用于已修改并已commit回退, 分三种情况

`HEAD` 表示当前提交, 上版本表示为`HEAD^`, 前100个版本表示为`HEAD~100`

1. `reset --hard`

删除工作空间改动, 撤销`git commit`, 撤销`git add .`

- 回退到上次commit

```sh
git reset --hard HEAD^
```

- 回退到任意版本

```sh
git reset --hard 某次commit_id
```

2. `reset --soft`

不删除工作空间改动, 撤销`git commit`, 不撤销`git add .`

```sh
git reset --soft HEAD^
```

回退后, 被修改文件会在**stage area**, 且文件自上次修改后内容不会删除

3. `reset --mixed`

不删除工作空间改动, 撤销`git commit`, 并撤销`git add .`

```sh
git reset --mixed HEAD^
```

回退后, 修改后文件会在**work space**, 且文件自上次修改后内容不会删除

## 分支

分支允许在代码库中创建多个开发线

```mermaid
gitGraph
    commit id: "main-1"
    commit id: "main-2"

    branch dev
    checkout dev
    commit id: "dev-1"
    checkout main
    
    branch fix
    checkout fix
    commit id: "fix-1"
```

每个分支本质上都是代码库一个快照, 可在不影响其他分支情况下独立进行开发和修改

```mermaid
flowchart LR
    %% 样式定义 (保持与前文一致的色彩系统)
    classDef center fill:#1e293b,stroke:#0f172a,stroke-width:3px,color:#ffffff,font-weight:bold,font-size:16px;
    
    classDef main_iso fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a,font-weight:bold;
    classDef sub_iso fill:#eff6ff,stroke:#93c5fd,stroke-width:1px,color:#1e40af;
    
    classDef main_mrg fill:#d1fae5,stroke:#059669,stroke-width:2px,color:#064e3b,font-weight:bold;
    classDef sub_mrg fill:#ecfdf5,stroke:#6ee7b7,stroke-width:1px,color:#065f46;
    
    classDef main_ver fill:#fef3c7,stroke:#d97706,stroke-width:2px,color:#78350f,font-weight:bold;
    classDef sub_ver fill:#fffbeb,stroke:#fcd34d,stroke-width:1px,color:#92400e;

    %% 中心节点
    Center{{"🌿 <b>Git Branch</b><br/>核心特点"}}

    %% 1. 代码隔离
    IsoMain("🛡️ <b>代码隔离</b>")
    IsoSub["🔒 提供安全的独立沙盒环境<br/>分支间代码独立，修改互不干扰"]

    %% 2. 代码合并
    MrgMain("🔀 <b>代码合并</b>")
    MrgMain2["🧩 通过 Merge / Rebase 操作<br/>将分支更改安全应用到目标分支"]

    %% 3. 并行开发 (原: 版本控制)
    VerMain("🚀 <b>并行开发</b>")
    VerSub["🛤️ 支持多分支工作流 (如 Git Flow)<br/>同时跟踪不同版本或开发阶段"]

    %% 连线关系
    Center ==>|1| IsoMain
    IsoMain --> IsoSub
    
    Center ==>|2| MrgMain
    MrgMain --> MrgMain2
    
    Center ==>|3| VerMain
    VerMain --> VerSub

    %% 应用样式
    class Center center;
    
    class IsoMain main_iso;
    class IsoSub sub_iso;
    
    class MrgMain main_mrg;
    class MrgMain2 sub_mrg;
    
    class VerMain main_ver;
    class VerSub sub_ver;

```

### 查看

#### 查看当前分支

初始化仓库时默认创建`master`(`main`)分支

```sh
git branch (-a, 表示所有分支)
```

- 示例, 查看仓库所有分支

```sh
git branch
```

#### 查看远程分支

```sh
git branch -r
```

### 创建

#### 仅创建分支

```sh
git branch 分支名
```

#### 创建分支并切换

```sh
git switch -c 分支名
```

- 示例, 创建dev分支并切换

```mermaid
gitGraph
    commit id: "main 1"
    commit id: "main 2"
    branch dev
    checkout dev
    commit id: "dev 1"
```

```sh
git switch -c dev
```

### 修改

#### 切换分支

```sh
git switch 分支名
```

- 示例, 切换回master分支

```sh
git switch master
```

#### 重命名

重命名当前分支

```sh
git branch -M 新分支名
```

### 删除

```sh
git branch -d 分支名
```

- 示例, 删除dev分支

```sh
git branch -d dev
```

示例输出

```sh
Deleted branch dev (was .....)
```

## 合并

合并代码时分两种情况,

(1) 完全合并, 需要另一个分支所有变动, 使用 `git merge`

(2) 选择合并, 只需要部分变动(某几次提交), 使用 `git cherry-pick`

### 完全合并

```sh
git merge 待合并分支名
```

#### 正常合并

```sh
git switch B分支

git merge A分支
```

- 示例, 将dev分支合并到master分支

master分支上提交

```mermaid
gitGraph
    commit id: "origin"
```

新建dev分支并切换, 进行两次提交

```mermaid
gitGraph
    commit id: "origin"
    branch dev
    checkout dev

    commit id: "modify first"
    commit id: "modify second"
```

切换回master分支, 合并dev分支所有提交

```mermaid
gitGraph
    commit id: "origin"
    branch dev
    switch dev
    commit id: "modify first"
    commit id: "modify second"
    switch main
    merge dev id: "merge dev"
```

#### 冲突情况

两个分支合并时可能会出现冲突错误

- 示例, 两分支修改同文件同位置, 导致合并冲突

master分支上创建 test.txt, 并提交

```c
// test.txt
master master master master master
```

```mermaid
gitGraph
    commit id: "master first commit"
```

创建dev分支, 修改 test.txt, 并提交

```c
// test.txt
dev dev dev dev dev
```

```mermaid
gitGraph
    commit id: "master first commit"
    branch dev
    switch dev
    commit id: "dev first commit"
```

切回master分支, 再修改test.txt 提交

```c
// test.txt
[master] upate upate upate upate upate
```

```mermaid
gitGraph
    commit id: "master first commit"
    branch dev
    switch dev
    commit id: "dev first commit"
    switch main
    commit id: "master second commit"
```

将dev分支合并到master分支

```mermaid
gitGraph
    commit id: "master first commit"
    branch dev
    switch dev
    commit id: "dev first commit"
    switch main
    commit id: "master second commit"
    merge dev type: REVERSE
```

提示冲突, 因两个分支都修改文件同一位置所导致, test.txt 内容如下

```sh
<<<<<<< HEAD
[master] upate upate upate upate upate
=======
dev dev dev dev dev
>>>>>>> dev
```

`<<<<<<<` 表示当前修改

`>>>>>>>` 表示传入修改

假如保留双方更改, 需手动修改冲突文件, 提交

```c
// test.txt
[master] upate upate upate upate upate
dev dev dev dev dev
```

```mermaid
gitGraph
    commit id: "master first commit"
    branch dev
    switch dev
    commit id: "dev first commit"
    switch main
    commit id: "master second commit"
    merge dev id: "merge dev"
```

### 选择合并

`git cherry-pick` 可选择其他分支单次、多次提交, 并作为新提交引入到当前分支

```sh
git cherry-pick 某次commit_id
```

- 示例, 将pick分支上某次提交合并到main分支

master分支上创建test.txt, 并提交

```c++
master master master master master
```

```mermaid
gitGraph
    commit id: "master 1 commit"
```

切换到pick分支, 对test.txt进行三次修改并提交

第一次修改

```diff
master master master master master
+ [pick] fix 1
```

第二次修改

```diff
master create origin
[pick] fix 1
+ [pick] fix 2
```

第三次修改

```diff
master create origin
[pick] fix 1
[pick] fix 2
+ [pick] fix 3
```

```mermaid
gitGraph
    commit id: "master 1 commit"
    branch pick
    switch pick
    commit id: "pick 1 commit"
    commit id: "pick 2 commit"
    commit id: "pick 3 commit"
```

(3) 选择合并

将pick分支 commit "pick 2 commit"提交合并到master分支

```mermaid
gitGraph
    commit id: "master 1 commit"
    branch pick
    switch pick
    commit id: "pick 1 commit"
    commit id: "pick 2 commit"
    commit id: "pick 3 commit"
    switch main
    cherry-pick id: "pick 2 commit"
```

如果有冲突, 需手动解决

后续可用`git cherry-pick --continue` 继续合并其他提交, 或用`git cherry-pick --abort` 放弃合并

#### 应用多提交

按顺序逐一应用提交到当前分支上

```sh
git cherry-pick commit1 commit2...
```

#### 应用范围提交

可指定提交范围应用

```sh
git cherry-pick 开始commit^结束commit
```

## 标签

`tag`(标签)标记特定`commit`, 用于记录项目开发过程中重要里程碑或发布版本

### 创建

#### 指定commit

```sh
git tag 标签名 (某次commit_id, 为空表示最新commit)
```

- 示例, 使用最新commit作标签

```sh
git tag v1.0
```

```mermaid
gitGraph
    commit id: "master 1 commit"
    commit id: "cherry-pick pick 2 commit" tag: "v1.0"
```

- 示例, 使用commit 52e3a21e149837607fffe8126...作标签v0.9

```sh
git tag v0.9 52e3a21e149837607fffe8126
```

```mermaid
gitGraph
    commit id: "master 1 commit" tag: "v0.9"
    commit id: "cherry-pick pick 2 commit" tag: "v1.0"
```

#### 添加描述信息

```sh
git tag -a 标签名 -m 描述信息 某次commit_id
```

### 删除

#### 删除本地tag

```sh
git tag -d 标签名
```

#### 删除远程tag

```sh
git push origin :refs/tags/标签名
```

### 查看

```sh
git tag
```

- 示例, 创建标签后查看, 终端显示

```sh
v0.9
v1.0
```

## 远程仓库

`remote repository`通常位于远程服务器, 如`github`、`gitlab`、`bitbucket`或`gitee`等代码托管平台

### 克隆

通过克隆可将`remote repository`内容复制到本地, 创建一个与之相同`local repository`

#### 完全克隆

克隆`remote repository`所有提交记录

```sh
git clone 仓库地址 (存放路径)
```

#### 拉取最近记录

```sh
git clone --depth 提交次数 参数地址
```

### 查看

#### 远程仓库地址

```sh
git remote -v
```

### 删除

#### 按名称删除

```sh
git remote rm 远程主机名
```

- 示例, 删除远程仓库origin

```sh
git remote rm origin
```

#### 按URL删除

```sh
git remote set-url --delete origin 远程仓库地址
```

### 添加

```sh
git remote add 远程主机名 远程仓库地址
```

### 拉取

从远程仓库获取更新, 使本地仓库保持同步

```sh
git pull 远程主机名 远程分支名:本地分支名
```

若远程分支是与当前分支合并, 可简写为`git pull`

#### 手动合并

`git pull` 过程可分解为,

(1) 通过`git fetch` 从远程主机将远程最新内容拉到本地, 用户可决定是否合并到本地分支中

```sh
git fetch 远程主机名 远程分支名
```

取回更新后, 会返回`FETCH_HEAD`, 指某个分支在服务器上最新状态

通过`git log -p FETCH_HEAD` 查看更新文件名、作者和时间、代码, 可以通过这些信息来判断是否产生冲突

(2) 通过`git merge` 将拉取下来最新内容合并到当前所在分支中, 若提示冲突, 需手动解冲突后提交

```sh
git merge FETCH_HEAD
```

### 推送

`git push` 将本地分支更改上传到远程仓库

```sh
git push 远程主机名 本地分支名:远程分支名
```

设存在远程仓库`http://192.168.1.128:3000/test_user/demo.git`

#### 推送已创建仓库

```sh
git remote add origin 仓库地址

git push origin 分支
```

#### 推送本地分支

```sh
git push 远程主机名 本地分支名:远程分支名
```

若本地分支与远程分支一致, 则可简写为`git push`

- 设置跟踪关系(可选)

为在以后推送中不再每次指定远程分支名, 可使用-u(或--set-upstream)标志来设置跟踪关系

```sh
git push -u 远程仓库名 本地分支名
```

设置跟踪关系后, 后续只需调用`git push`即可将更改推送到所关联远程分支

- 同时推送到多仓库

添加多个远程仓库地址, 就可同步`push`到多个库

```sh
git remote set-url --add origin 远程仓库地址
```

#### 推送本地标签

```sh
git push origin 分支名 --tags
```

## 子模块

子模块(`git submodules`)允许将一个`git`仓库作为另一个`git`仓库的子目录

```mermaid
flowchart LR
    %% 样式定义 (保持与前文一致的色彩系统)
    classDef center fill:#1e293b,stroke:#0f172a,stroke-width:3px,color:#ffffff,font-weight:bold,font-size:16px;
    
    classDef main_dep fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a,font-weight:bold;
    classDef sub_dep fill:#eff6ff,stroke:#93c5fd,stroke-width:1px,color:#1e40af;
    
    classDef main_ver fill:#d1fae5,stroke:#059669,stroke-width:2px,color:#064e3b,font-weight:bold;
    classDef sub_ver fill:#ecfdf5,stroke:#6ee7b7,stroke-width:1px,color:#065f46;
    
    classDef main_reuse fill:#fef3c7,stroke:#d97706,stroke-width:2px,color:#78350f,font-weight:bold;
    classDef sub_reuse fill:#fffbeb,stroke:#fcd34d,stroke-width:1px,color:#92400e;
    
    classDef main_iso fill:#ede9fe,stroke:#7c3aed,stroke-width:2px,color:#4c1d95,font-weight:bold;
    classDef sub_iso fill:#f5f3ff,stroke:#c4b5fd,stroke-width:1px,color:#5b21b6;

    %% 中心节点
    Center{{"🎯 <b>Git Submodule</b><br/>核心作用"}}

    %% 1. 依赖管理分支
    DepMain("📦 <b>依赖管理</b>")
    DepSub["🔗 将三方库作为子模块包含<br/>方便管理复杂的依赖关系"]

    %% 2. 版本控制分支
    VerMain("🏷️ <b>版本控制</b>")
    VerSub["🔄 可单独跟踪和更新<br/>子模块的特定 Commit 版本"]

    %% 3. 代码复用分支
    ReuseMain("♻️ <b>代码复用</b>")
    ReuseSub["📂 多个项目共享相同代码库<br/>无需物理复制代码文件"]

    %% 4. 隔离性分支
    IsoMain("🛡️ <b>隔离性</b>")
    IsoSub["🔒 在不影响主项目历史的情况下<br/>独立更新或修改子模块"]

    %% 连线关系 (使用加粗线突出主干)
    Center ==>|1| DepMain
    DepMain --> DepSub
    
    Center ==>|2| VerMain
    VerMain --> VerSub
    
    Center ==>|3| ReuseMain
    ReuseMain --> ReuseSub
    
    Center ==>|4| IsoMain
    IsoMain --> IsoSub

    %% 应用样式
    class Center center;
    
    class DepMain main_dep;
    class DepSub sub_dep;
    
    class VerMain ver_main;
    class VerSub sub_ver;
    
    class ReuseMain main_reuse;
    class ReuseSub sub_reuse;
    
    class IsoMain main_iso;
    class IsoSub sub_iso;

```

### 初始化

#### 自动拉取

使用`--recursive`在克隆项目同时拉取子模块

```sh
git clone --recursive 项目地址
```

#### 手动拉取

若克隆项目时未拉取子模块, 可在主项目根目录手动拉取

```sh
git submodule init

git submodule update --recursive
```

### 查看

```sh
git submodule
```

### 添加

```sh
git submodule add 子模块仓库地址 (子模块路径)
```

### 修改

#### 更新

更新子模块以获取最新改动

```sh
git submodule update --remote
```

#### 提交

子模块修改后, 与普通项目提交方式一致

### 删除

#### 移除子模块

```sh
git submodule deinit -f 子模块本地路径
```

#### 删除子模块目录

```sh
git rm 子模块本地路径
```

## 配置

### 下载

[Git客户端官网下载](https://git-scm.com/downloads)

### 查看

#### 系统配置

```sh
git config --system --list
```

#### 用户配置

```sh
git config --global --list
```

### 修改

#### 用户信息

- 配置用户名(必须)

```sh
git config --global user.name "用户名"
```

- 配置邮箱(必须)

```sh
git config --global user.email "邮箱"
```

#### 设置

- 配置编辑器

```sh
git config --global core.editor "code -w"
```

- 防止中文乱码

```sh
git config --global core.quotepath false
```

- 启用文件大小写敏感

```sh
git config --global core.ignorecase false
```

- 设置代理

```sh
git config --global http.proxy 代理地址
```

- 取消代理

```sh
git config --global --unset http.proxy
```

## 使用

### 本地使用

- 示例, 本地新建项目git_demo, 进入git_demo/后执行

```sh
git init
```

`git_demo/`中会出现`.git/`隐藏目录

新建hello.md文件

```md
# Hello World
```

提交

```sh
git add .
```

```sh
git commmit -m "初次提交"
```

### github使用

#### 设置

注册github账号

- 配置公钥

github设置公钥后, 上传下载即可免输入密码

复制本地用户`.ssh/`公钥`id_rsa.pub`值, 添加到github settings中SSH and GPG keys中

#### 上传

本地项目上传到github仓库上

```mermaid
flowchart TD
    %% 样式定义 (延续前文色彩体系)
    classDef startEnd fill:#f8fafc,stroke:#64748b,stroke-width:3px,color:#334155,font-weight:bold,font-size:16px;
    classDef ws fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a,font-weight:bold;
    classDef sa fill:#fef3c7,stroke:#d97706,stroke-width:2px,color:#78350f,font-weight:bold;
    classDef lr fill:#d1fae5,stroke:#059669,stroke-width:2px,color:#064e3b,font-weight:bold;
    classDef rr fill:#ede9fe,stroke:#7c3aed,stroke-width:2px,color:#4c1d95,font-weight:bold;
    classDef decision fill:#ffe4e6,stroke:#e11d48,stroke-width:2px,color:#9f1239,font-weight:bold;

    Start(("🚀 开始开发"))
    End(("🏁 同步完成"))

    subgraph Local ["💻 阶段一：本地工作流 (Local Workflow)"]
        direction TB
        WS["📝 <b>工作区修改</b><br/>(Work Space)"]
        SA["📦 <b>暂存区快照</b><br/>(Stage Area)"]
        LR["🗄️ <b>本地库提交</b><br/>(Local Repo)"]
        
        WS -- "1. <code>git add .</code>" --> SA
        SA -- "2. <code>git commit -m 'msg'</code>" --> LR
    end

    Decision{"🔗 是否首次推送？<br/>(本地是否已关联远程仓库)"}

    subgraph Remote ["☁️ 阶段二：远程推送 (Remote Push)"]
        direction TB
        SetRemote["⚙️ <b>配置远程地址</b><br/><code>git remote add origin &lt;url&gt;</code>"]
        PushFirst["🚀 <b>首次推送并绑定上游</b><br/><code>git push -u origin &lt;branch&gt;</code>"]
        PushNormal["🔄 <b>常规推送</b><br/><code>git push</code>"]
        RR["🌐 <b>远程仓库更新</b><br/>(Remote Repo)"]
    end

    %% 连线
    Start --> WS
    LR --> Decision
    
    Decision -- "Y (首次)" --> SetRemote --> PushFirst --> RR
    Decision -- "N (非首次)" --> PushNormal --> RR
    
    RR --> End

    %% 应用样式
    class Start,End startEnd;
    class WS ws;
    class SA sa;
    class LR lr;
    class RR rr;
    class Decision decision;
```

后续提交可直接使用`git push`推送

#### 下载

将github上项目下载(更新)到本地

```mermaid
flowchart TD
    %% 样式定义 (延续前文色彩体系)
    classDef startEnd fill:#f8fafc,stroke:#64748b,stroke-width:3px,color:#334155,font-weight:bold,font-size:16px;
    classDef decision fill:#ffe4e6,stroke:#e11d48,stroke-width:2px,color:#9f1239,font-weight:bold;
    
    classDef cloneNode fill:#ede9fe,stroke:#7c3aed,stroke-width:2px,color:#4c1d95,font-weight:bold;
    classDef pullNode fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a,font-weight:bold;
    classDef fetchNode fill:#d1fae5,stroke:#059669,stroke-width:2px,color:#064e3b,font-weight:bold;

    Start(("🚀 开始获取代码"))
    End(("🏁 同步完成"))

    Decision1{"本地是否已有<br/>该项目的 Git 仓库？"}

    subgraph CloneFlow ["📥 场景一：首次获取项目"]
        direction TB
        Clone["🌐 <b>克隆远程仓库</b><br/><code>git clone &lt;url&gt;</code><br/><i>自动初始化本地仓库、下载所有历史并关联远程</i>"]
    end

    Decision2{"选择日常同步策略<br/>(本地已有仓库)"}

    subgraph PullFlow ["⚡ 场景二：快捷同步策略"]
        direction TB
        Pull["🔄 <b>拉取并自动合并</b><br/><code>git pull</code><br/><i>等同于 fetch + merge<br/>适合工作区干净、追求效率的场景</i>"]
    end

    subgraph FetchFlow ["🛡️ 场景三：安全审查策略 (团队协作推荐)"]
        direction TB
        Fetch["📥 <b>1. 安全拉取更新</b><br/><code>git fetch</code><br/><i>仅下载更新到本地远程跟踪分支<br/>绝对不修改当前工作区代码</i>"]
        Review["🔍 <b>2. 审查代码差异</b><br/><code>git log</code> / <code>git diff</code><br/><i>查看远程到底更新了什么内容</i>"]
        Merge["🔀 <b>3. 手动合并或变基</b><br/><code>git merge</code> 或 <code>git rebase</code><br/><i>确认无误后，将更新合并到本地分支</i>"]
        
        Fetch --> Review --> Merge
    end

    %% 连线
    Start --> Decision1
    
    Decision1 -- "N (没有，首次获取)" --> Clone --> End
    
    Decision1 -- "Y (已有，日常更新)" --> Decision2
    
    Decision2 -- "追求快捷<br/>(工作区干净)" --> Pull --> End
    Decision2 -- "追求安全<br/>(需审查/防冲突)" --> FetchFlow --> End

    %% 应用样式
    class Start,End startEnd;
    class Decision1,Decision2 decision;
    class Clone cloneNode;
    class Pull pullNode;
    class Fetch,Review,Merge fetchNode;

```

- 克隆

首次将项目下载到本地

```sh
git clone 仓库地址
```

#### PR

`PR`(`Pull Request`)功能允许个人向其他项目贡献代码, 通常用于团队协作和开源项目贡献中

当开发者修改他人代码后, 可以通过PR将修改告知给代码原作者, 请求他合并这些修改

```mermaid
flowchart TD
    %% 样式定义 (延续前文色彩体系)
    classDef contributor fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a,font-weight:bold;
    classDef platform fill:#ede9fe,stroke:#7c3aed,stroke-width:2px,color:#4c1d95,font-weight:bold;
    classDef success fill:#d1fae5,stroke:#059669,stroke-width:3px,color:#064e3b,font-weight:bold,font-size:16px;
    classDef bgBox fill:#f8fafc,stroke:#94a3b8,stroke-width:2px,stroke-dasharray: 5 5;

    subgraph Contributor ["👨‍💻 阶段一：贡献者本地与个人远程操作"]
        direction TB
        Fork["🍴 <b>1. Fork 仓库</b><br/>在 GitHub/GitLab 平台上<br/>将原仓库复制到自己的账号下"]
        Clone["💻 <b>2. Clone & 创建特性分支</b><br/><code>git clone &lt;your_fork_url&gt;</code><br/><code>git checkout -b feature-x</code>"]
        Push["🚀 <b>3. 修改代码 & 推送</b><br/>在本地编写代码并 commit<br/><code>git push origin feature-x</code>"]
        
        Fork --> Clone --> Push
    end

    subgraph Platform ["☁️ 阶段二：平台交互与维护者审查"]
        direction TB
        PR["🔀 <b>4. 创建 PR (Pull Request)</b><br/>在平台上发起合并请求：<br/>请求将 Fork 仓库的 <code>feature-x</code> 分支<br/>合并到原仓库的 <code>main</code> 分支"]
        Review["🔍 <b>5. 代码审查 (Code Review)</b><br/>维护者审查代码、CI 自动运行测试<br/>若有问题，贡献者在本地修改后再次 push<br/><i>(PR 会自动更新)</i>"]
        Merge["✅ <b>6. 合并代码 (Merge PR)</b><br/>审查通过，维护者点击 Merge<br/>代码正式合入原项目主干，PR 关闭"]
        
        PR --> Review --> Merge
    end

    %% 跨区连线
    Push ==>|"触发平台操作<br/>(在网页端点击 New PR)"| PR

    %% 应用样式
    class Contributor,Platform bgBox;
    class Fork,Clone,Push contributor;
    class PR,Review platform;
    class Merge success;

```

## 工具

### repo

```sh
mkdir -p ~/repo
cd ~/repo
git clone https://mirrors.tuna.tsinghua.edu.cn/git/git-repo

cd ~/repo/git-repo
sudo chmod +x ./repo

sudo cp ./repo /usr/bin/
```

#### 设置环境变量

变量`~/.bashrc`, 追加

```sh
export REPO_URL='https://mirrors.tuna.tsinghua.edu.cn/git/git-repo/'
```

```sh
source ~/.bashrc
```