...
约束(Constraint)用于限制加入表的数据，以保证数据的准确性和可靠性。最常见的是**主键(Primary Key)**和**外键(Foreign Key)**
...
以下是一个典型的学生-课程-成绩(Stu-Course-Score)模型
...
同学编号为 X-47-W, 其量子力学成绩为 80 分, 插入 Score 表
