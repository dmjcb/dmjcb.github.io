...
L4D2 服务端体积较大, 建议在宿主机上预先下载, 然后通过数据卷挂载到 Docker 容器中
...
- 获取 `Metamod` 和 `SourceMod`

前往 Metamod 官网 和 SourceMod 官网 下载最新的 Linux 版本

- 解压到游戏目录

将下载的压缩包解压到宿主机的 l4d2_server/left4dead2/ 目录下(会生成 addons 和 cfg 等文件夹, 直接覆盖合并)

- 配置 `Metamod`

在 left4dead2/addons/ 目录下创建 metamod.vdf 文件, 内容如下
