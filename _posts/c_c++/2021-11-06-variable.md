...
在 C/C++ 中, 变量(Variable)本质上是对计算机内存中某块存储区域的抽象和命名

如果把计算机的内存想象成一个巨大的、由无数个"小格子"组成的仓库, 那么变量就是这个仓库里贴了标签的盒子

一个完整的变量概念包含三个核心要素：

- 名称(name / identifier): 即"标签"(如 a, count), 用于在代码中唯一标识和访问这块内存

- 类型(type): 决定了"盒子"的大小(占用多少字节)以及里面存放的二进制数据该如何被解释(例如 int 代表 4 字节的整数, double 代表 8 字节的浮点数)

- 值(value): 盒子里实际存放的数据, 在变量的生命周期内, 其值通常是可以被修改的(const 修饰的只读变量除外)

>💡 进阶认知：在 C++ 标准的严谨术语中, 更倾向于使用对象(Object) 一词
> 内存中任何一块被分配了类型的存储区域都是"对象", 而"变量"是我们赋予这个对象的名字
> 我们说"定义一个变量", 本质上就是"向系统申请一块内存, 并给它贴上名字和类型的标签"
...
flowchart LR
    %% 样式定义
    classDef strBox fill:#f0fdf4,stroke:#22c55e,stroke-width:2px,color:#166534;
    classDef ptrBox fill:#eff6ff,stroke:#3b82f6,stroke-width:2px,color:#1e3a8a;
    classDef cell fill:#ffffff,stroke:#cbd5e1,stroke-width:1px,color:#0f172a,font-family:monospace,font-size:14px;
    classDef link stroke:#3b82f6,stroke-width:2px;

    subgraph StrEnv ["📦 字符串 "HELLO""]
        direction TB
        a0["<b>H</b><br><code>0x...f02</code>"]
        a1["<b>E</b><br><code>0x...f03</code>"]
        a2["<b>L</b><br><code>0x...f04</code>"]
        a3["<b>L</b><br><code>0x...f05</code>"]
        a4["<b>O</b><br><code>0x...f06</code>"]
        a5["<b>\0</b><br><code>0x...f07</code>"]
        a0 ~~~ a1 ~~~ a2 ~~~ a3 ~~~ a4 ~~~ a5
    end

    subgraph PtrEnv ["🔗 指针 char* p"]
        direction TB
        v["<b>p</b><br><code>value: 0x...f02</code>"]
    end
