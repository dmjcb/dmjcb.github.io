...
Clang 是基于 LLVM 架构的 C/C++/Objective-C 编译器，以编译速度快、报错信息友好著称
...
tar xvf llvmorg-17.0.6.tar.gz
cd llvm-project-llvmorg-17.0.6

# 2. 配置 CMake (注意源码目录是 llvm 子目录)
...
- 预处理 (Pre-processing)：处理宏定义、头文件包含、条件编译

```sh
gcc main.c -E -o main.i
```

- 编译 (Compilation): 词法/语法分析，生成汇编代码

```sh
gcc main.i -S -o main.s
```

- 汇编(Assembly):将汇编代码转为机器指令（目标文件）

```sh
gcc main.s -c -o main.o
```

- 链接 (Linking):将多个目标文件与库文件合并，生成可执行文件

```sh
gcc main.o -o main
```

直接使用 `gcc main.c -o main` 时，编译器会自动在后台依次执行这四个阶段
