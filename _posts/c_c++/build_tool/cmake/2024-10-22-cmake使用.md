...
flowchart LR
    %% 样式定义 (延续前文语义化配色体系)
    classDef cfgFile fill:#f1f5f9,stroke:#64748b,stroke-width:2px,color:#334155,font-weight:bold;
    classDef cmakeNode fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a,font-weight:bold;
    classDef buildFile fill:#fef3c7,stroke:#d97706,stroke-width:2px,color:#78350f,stroke-dasharray: 5 5;
    classDef buildTool fill:#d1fae5,stroke:#059669,stroke-width:2px,color:#064e3b,font-weight:bold;
    classDef outBin fill:#1e293b,stroke:#0f172a,stroke-width:3px,color:#ffffff,font-weight:bold;

    %% 节点定义
    Cfg["📄 <b>CMakeLists.txt</b><br>跨平台构建配置文件<br>定义源文件、依赖、编译选项"]:::cfgFile

    Cmake["⚙️ <b>CMake</b><br>① 配置 (Configure)<br>检查环境、解析依赖<br>② 生成 (Generate)<br>输出平台构建文件"]:::cmakeNode

    BuildFile["📜 <b>构建文件</b><br><code>Makefile / Ninja</code><br><code>VS Project</code>"]:::buildFile

    BuildTool["🔨 <b>构建工具</b><br>③ 构建 (Build)<br><code>make / ninja</code><br><code>cmake --build</code>"]:::buildTool

    OutBin["🚀 <b>可执行程序 / 库</b><br><code>a.out / app.exe</code><br><code>libtest.so / .dll</code>"]:::outBin

    %% 连线关系
    Cfg ==>|"① 读取配置"| Cmake
    Cmake ==>|"② 生成"| BuildFile
    BuildFile ==>|"③ 驱动"| BuildTool
    BuildTool ==>|"编译链接"| OutBin
...
flowchart LR
    %% 样式定义 (延续前文语义化配色体系)
    classDef rootBox fill:#1e293b,stroke:#0f172a,stroke-width:3px,color:#ffffff,font-weight:bold,font-size:15px;
    classDef cmdBox fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a,font-weight:bold;
    classDef paramBox fill:#f8fafc,stroke:#94a3b8,stroke-width:1.5px,color:#475569,stroke-dasharray: 5 5;

    %% ================= 节点定义 =================
    
    %% 起点：核心语法
    X["📘 <b>CMake 核心指令</b><br><code>CMakeLists.txt</code>"]:::rootBox

    %% 分支 1：版本要求
    A["<b>cmake_minimum_required</b><br>指定最低 CMake 版本"]:::cmdBox
    A1["<b>版本参数</b><br><code>VERSION 3.15</code>"]:::paramBox

    %% 分支 2：项目定义
    B["<b>project</b><br>定义项目名称与语言"]:::cmdBox
    B1["<b>项目参数</b><br><code>MyApp CXX</code>"]:::paramBox

    %% 分支 3：变量设置
    C["<b>set</b><br>定义变量或设置编译参数"]:::cmdBox
    C1["<b>变量赋值</b><br><code>CMAKE_CXX_STANDARD 17</code>"]:::paramBox

    %% 分支 4：目标声明
    D["<b>add_executable / add_library</b><br>声明生成的可执行文件或库"]:::cmdBox
    D1["<b>目标与源文件</b><br><code>app main.cpp</code>"]:::paramBox

    %% 分支 5：安装规则
    E["<b>install</b><br>定义安装规则与目标路径"]:::cmdBox
    E1["<b>安装配置</b><br><code>TARGETS app DESTINATION bin</code>"]:::paramBox
...
flowchart LR
    ...
    %% 节点定义
    A["📄 <b>CMakeLists.txt</b><br>项目构建配置文件<br>定义源文件与编译规则"]:::cfgFile
    
    B["⚙️ <b>CMake</b><br>解析配置并生成<br>平台专属构建文件"]:::cmakeNode
    
    C["📜 <b>构建文件</b><br><code>Makefile / Ninja</code><br>驱动编译器执行构建"]:::buildFile
