...
flowchart LR
    %% 节点定义
    Cfg["<b>CMakeLists.txt</b><br>跨平台构建配置文件<br>定义源文件、依赖、编译选项"]

    Cmake["<b>CMake</b><br>① 配置 (Configure)<br>检查环境、解析依赖<br>② 生成 (Generate)<br>输出平台构建文件"]

    BuildFile["<b>构建文件</b><br><code>Makefile / Ninja</code><br><code>VS Project</code>"]

    BuildTool["<b>构建工具</b><br>③ 构建 (Build)<br><code>make / ninja</code><br><code>cmake --build</code>"]

    OutBin["<b>可执行程序 / 库</b><br><code>a.out / app.exe</code><br><code>libtest.so / .dll</code>"]
...
flowchart LR
    %% 节点定义
    A["<b>CMakeLists.txt</b><br>项目构建配置文件<br>定义源文件与编译规则"]
    
    B["<b>CMake</b><br>解析配置并生成<br>平台专属构建文件"]
    
    C["<b>构建文件</b><br><code>Makefile / Ninja</code><br>驱动编译器执行构建"]
