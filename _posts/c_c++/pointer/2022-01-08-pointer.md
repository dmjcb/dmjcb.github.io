...
在 `c/c++` 中, 每个变量(`variable/object`)在源码层面和运行时层面具有以下核心属性:

- `identifier`(标识符)

源码中的变量名

仅存在于编译期的符号表中, 编译链接后会被替换为具体的内存地址或寄存器偏移量, 运行时不占用内存
...
```mermaid
graph LR
    A["标识符 (Identifier)"] -->|编译期: 符号解析与重定位| B("内存地址 (Address)")
    B -->|运行期: CPU 寻址定位| C{"值 (Value)"}
    
    style A fill:#f9f2f4,stroke:#c7254e,stroke-width:1px
    style B fill:#e8f4f8,stroke:#31708f,stroke-width:1px
...
指针变量去掉变量名后是指针类型, 指针类型决定了解引用时读取的字节数以及指针算术运算的步长
...
> 关于指针的大小
>
> 在主流的冯·诺依曼架构(统一地址空间, 如 x86/x64)下, 普通数据指针的大小只与系统位数有关(32位为4字节, 64位为8字节), 与指向类型无关
>
> 注：在某些特殊架构(如哈佛架构)或 C++ 的指向类成员的指针(Member Pointers)中, 指针大小可能与类型有关。
...
指向指针的指针, 其值是另一个指针变量的地址

常用于在函数内部修改外部指针的指向, 或表示二维数组
