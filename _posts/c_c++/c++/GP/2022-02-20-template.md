...
编译器会通过模板参数推导 (`template argument deduction`) 自动推断类型, 或者由开发者显式指定
...
与函数模板不同, `c++17` 之前类模板无法自动推导参数类型; 

`c++17` 引入了类模板参数推导 (CTAD, `class template argument deduction`), 使得在特定条件下(如提供推导指引或构造函数参数足够明确)可以省略模板参数
...
class Composer {
public:
    // C++17 起, 若构造函数参数类型明确, 可触发 CTAD
    Composer(T x, T y) : mX(x), mY(y) {}

    T get_max() const { return mX > mY ? mX : mY; }
private:
    T mX, mY;
};
...
int main() {
    // 显式指定 (C++17 前必须)
    Composer<int> com1(1, 2); 
    
    // CTAD 自动推导 (C++17 起支持)
    Composer com2(1.5, 2.5); // 推导为 Composer<double>

    std::cout << com2.get_max() << '\n';
...
c++17 引入了折叠表达式 (`fold expressions`), 彻底消灭了冗长的递归解包代码, 这是现代 c++ 处理参数包的标准做法

```c++
template<typename... Args>
void print_modern(Args... args) {
    // 一元右折叠：将 args 中的所有元素用 << 连接起来
    (std::cout << ... << args) << '\n'; 
}

int main() {
    print_modern(1, 2.5, "Hello", " C++17");
    return 0;
}
```
...
模板特化允许为某些特定的类型提供不同于通用模板的定制化实现, 分为全特化和偏特化

当存在多个特化版本时, 编译器会按照以下优先级进行匹配：

```mermaid
flowchart LR
    A["主模板<br/>Primary Template"] -->|匹配度最低| D((实例化))
    B["偏特化<br/>Partial Specialization"] -->|匹配度中等| D
    C["全特化<br/>Full Specialization"] -->|匹配度最高| D

    classDef primary fill:#ffebee,stroke:#c62828,stroke-width:2px;
...
template<typename T>
struct TypeAnalyzer {
    void analyze() { std::cout << "Generic: " << typeid(T).name() << '\n'; }
};

// 2. 全特化 (针对 std::string)
template<>
struct TypeAnalyzer<std::string> {
    void analyze() { std::cout << "Specialized for std::string\n"; }
};

// 3. 偏特化 (针对所有指针类型 T*)
template<typename T>
struct TypeAnalyzer<T*> {
    void analyze() { std::cout << "Pointer to: " << typeid(T).name() << '\n'; }
};
...
int main() {
    ...
    TypeAnalyzer<std::string> t2;
    t2.analyze(); // Specialized
    
    TypeAnalyzer<int*> t3;
    t3.analyze(); // Pointer to
    
    return 0;
}
```

## 类型萃取与推导 (type traits & deduction)
