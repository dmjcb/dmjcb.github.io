...
当一个 C/C++ 程序被操作系统加载并运行时, 操作系统会为其分配一个独立的虚拟地址空间。在主流的 64 位 Linux 系统下, 其内存布局通常如下所示

```mermaid
graph TB;
    classDef kernel fill:#f9f2f4,stroke:#c7254e,stroke-width:2px,color:#333;
    classDef stack fill:#e8f4f8,stroke:#31708f,stroke-width:2px,color:#333;
    classDef heap fill:#dff0d8,stroke:#3c763d,stroke-width:2px,color:#333;
    classDef data fill:#fcf8e3,stroke:#8a6d3b,stroke-width:2px,color:#333;
    classDef text fill:#f5f5f5,stroke:#999,stroke-width:2px,color:#333;

    subgraph VirtualMemory ["64位进程虚拟地址空间 (高地址 -> 低地址)"]
        direction TB
        Kernel["🛡️ 内核空间 (Kernel Space)<br/>用户态不可直接访问"]
        Stack["📉 栈区 (Stack)<br/>局部变量、函数调用帧<br/><i>向低地址增长</i>"]
        Mmap["🗂️ 内存映射区 (mmap)<br/>动态链接库、大内存分配、文件映射"]
        Heap["📈 堆区 (Heap)<br/>动态分配的内存 (malloc/new)<br/><i>向高地址增长</i>"]
        BSS["📦 BSS 段<br/>未初始化的全局/静态变量<br/><i>默认清零</i>"]
        Data["📦 Data 段<br/>已初始化的全局/静态变量"]
        Rodata["🔒 只读数据段 (Rodata)<br/>常量字符串、const 全局变量"]
        Text["⚙️ 代码段 (Text)<br/>编译后的机器指令"]
    end

    class Kernel kernel;
    class Stack stack;
    class Heap heap;
    class Data,BSS,Rodata data;
    class Text text;
```

> 💡 架构提示: 栈向低地址增长、堆向高地址增长是 x86/x64 等主流架构的实现, 并非 C/C++ 标准强制规定
...
通过移动栈顶指针(如 RSP 寄存器)实现, 速度极快(纳秒级)

- 存储内容: 局部变量、函数参数、返回地址、寄存器上下文(构成栈帧 `stack frame`)

- 生命周期: 随函数调用而创建, 随函数返回而自动销毁(LIFO 原则)

- 限制: 容量有限(linux 默认通常为 8MB), 递归过深或分配超大局部数组会导致 栈溢出 (`stack overflow`)

#### 堆区 (Heap)

- 分配方式: 由程序员手动管理(`malloc`/`free` 或 `new`/`delete`)

底层由 C 运行时库(如 `glibc` 的 `ptmalloc`)维护, 涉及复杂的链表操作, 速度较慢(微秒级)

- 存储内容: 生命周期需要跨越函数调用的对象、大型数据结构

- 限制: 容量受限于系统虚拟内存。频繁分配/释放容易产生内存碎片

#### 全局/静态区 (Data & BSS)

- Data 段: 存储已初始化的全局变量和 static 变量

- BSS 段: 存储未初始化的全局变量和 static 变量(OS 加载时会自动清零)

- 生命周期: 程序启动时分配, 程序终止时由 OS 回收

#### 常量区 (Rodata) & 代码区 (Text)

- Rodata: 存储字符串字面量(如 "Hello")和 const 修饰的全局常量

通常被标记为只读, 修改会导致段错误 (Segmentation Fault)
