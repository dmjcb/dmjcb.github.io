...
(6) `-> libpcre2-16.so.0.11.2` 链接指向目标, `/lib/x86_64-linux-gnu/libpcre.so.3`是一个指向`libpcre2-16.so.0.11.2`符号链接

当系统或程序尝试访问`libpcre2-16.so.0` 时, 实际上会访问到`libpcre2-16.so.0.11.2`
...
目录文件用于存储和管理其他文件和目录, 类似`windows`中目录, 包含一个或多个文件和子目录列表
